...
public class Reporter : MonoBehaviour {
	...
	public class Sample
	{
		...
		public float 	memory ;
		public float 	fps ;
		public float  	fpsToDisplay ;
		public string FormatFpsToDisplay() { return fpsToDisplay.ToString("0.000"); }
	...
	public bool show { get { return ReporterGUI.inst != null; } }
	//collapse logs
	bool collapse ;
	...
	float  gcTotalMemory ;
	public System.Func<string> GetUserData;
	...
	private int sceneIdx { get { return SceneManager.GetActiveScene().buildIndex; } }
	private string sceneName { get { return SceneManager.GetActiveScene().name; } }
	...
	void addSample(){
		...
		sample.fps = fps ;
		sample.fpsToDisplay = fpsToDisplay ;
		sample.loadedScene = (byte)sceneIdx;
	...
	void DrawInfo()
	{
		...
		GUILayout.Space( size.x);
		GUILayout.Label( FormatFpsToDisplay() , nonStyle , GUILayout.Height(size.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
		...
		GUILayout.Space( size.x);
		if (GetUserData != null)
			GUILayout.Label( GetUserData() , nonStyle , GUILayout.Height(size.y));
		else
			GUILayout.Label( "" , nonStyle , GUILayout.Height(size.y));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
	...
	void drawInfo_enableDisableToolBarButtons()
	{
		...
		tempRect = GUILayoutUtility.GetLastRect();
		GUI.Label( tempRect , FormatFpsToDisplay() , lowerLeftFontStyle );
	...
	void DrawLogs()
	{
		...
		for( int i = startIndex ; (startIndex + index) < endIndex ; i++ )
		{
			...
			fpsRect = countRect ;
			if( showFps ){
				tempContent.text = sample.FormatFpsToDisplay();
			...
			if( log == selectedLog ){
				...
				if( showFps ){
					...
					GUI.Label( fpsLabelRect , sample.FormatFpsToDisplay() ,currentLogStyle   );
				}
	...
	void Update()
	{
		...
		if( string.IsNullOrEmpty( scenes[ sceneIdx ] ))
			scenes[ sceneIdx ] = sceneName;
